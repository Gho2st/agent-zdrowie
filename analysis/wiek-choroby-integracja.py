...
df = pd.read_excel(
    plik_excel, sheet_name="Liczba odpowiedzi 1", usecols=[1, 19]
)  # 1 = Wiek, 19 = Integracja
...
print(
    "Tabela 1. Skłonność do integracji z systemami jak Google Fit według grup wiekowych (N=123)"
)
print(contingency)
...
df = pd.read_excel(
    plik_excel, sheet_name="Liczba odpowiedzi 1", usecols=[3, 19]
)  # 3 = Choroby przewlekłe, 19 = Integracja

df.columns = ["Choroby", "Integracja"]

# Filtruj tylko Tak/Nie
